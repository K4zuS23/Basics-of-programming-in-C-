...
namespace TestNetStandart
{
    public class Program
    {
		...
		class ProviderCompany
		{
			...
			public override string ToString()
			{
				return $"Компанія постачальник:\t{Name}\n";
			}
		}
		//У кожного поставленого товару є компанія постачальник.
		private Dictionary<Product, ProviderCompany> ProductProviders { get; set; } = new Dictionary<Product, ProviderCompany>();
		...
		{
			Console.WriteLine($"Оберіть потрібний товар:");
			OutputProducts();
			...
			// Якщо користувач обрав неіснуючий товар
			if (selectedDocIndex > ProductProviders.Keys.Count)
			{
				Console.WriteLine($"Ви обрали неіснуючий товар.");
				return null;
			}
		...
		private void OutputProducts()
		{
			...
			{
				Console.WriteLine("Поки що не було додано жодного товару.");
			}
		}
		...
		private void UpdateProductPrice()
		{
			...
			Console.WriteLine($"Введіть нову ціну для обраного продукту:");
			short newPrice;
			while (!short.TryParse(Console.ReadLine(), out newPrice))
				Console.WriteLine($"Введіть коректне значення.\n");
			product.Price = newPrice;
			Console.WriteLine("Ціна товару була змінена.");
		}
		...
		private void Execute()
		{
			...
			switch (selectedOption)
			{
				...
				case ProductOptions.DeleteProduct:
					...
					else Console.WriteLine($"Ви не можете видалити якийсь товар. Поки що не було додано жодного товару.");
					break;
				case ProductOptions.UpdateProductPrice:
					if (ProductProviders.Count > 0) UpdateProductPrice();
					else Console.WriteLine($"Ви не можете оновити ціну якогось товару. Поки що не було додано жодного товару.");
					break;
